...
function isPasswordValid(rule = "") {
  const parsed = rule.split(" ");
  const upperLimit = parseInt(parsed[0].split("-")[1], 10);
  const lowerLimit = parseInt(parsed[0].split("-")[0], 10);
  const char = parsed[1].split(":")[0].trim();
  const password = parsed[2].trim();
  const password__iter = password.split("");
  let charRepeatCount = 0;

  for (let i = 0; i < password__iter.length; i++) {
    if (char === password__iter[i]) charRepeatCount++;
  }

  return charRepeatCount >= lowerLimit && charRepeatCount <= upperLimit;
}

for (let i = 0; i < input.length; i++) {
  isPasswordValid(input[i]) ? counter++ : null;
}
...
function bloodyShopkeeper(rule) {
  const parsed = rule.split(" ");
  const firstPos = parseInt(parsed[0].split("-")[0], 10);
  const secondPos = parseInt(parsed[0].split("-")[1], 10);
  const char = parsed[1].split(":")[0].trim();
  const password = parsed[2].trim();
  const password__iter = password.split("");
  let flag = false;
  ...
  if (
    password__iter[firstPos - 1] === char &&
    password__iter[secondPos - 1] !== char
  ) {
    flag = true;
  }
...
for (let i = 0; i < input.length; i++) {
  bloodyShopkeeper(input[i]) ? counter2++ : null;
}
